...
contract PineToken is ERC20('Atropine', 'PINE') {
    ...
    /* 
        ...
        14 after the newMinter was set. 
    */
    ...
    uint256 public initialized;
     ...
     constructor(uint256 _preMintAmount) public {
        // pre minted amount, used to add liquidity
        _mint(msg.sender, _preMintAmount);
    }
    
    function initialize(address _masterChef) public {
        require(initialized == 0, "PINE: Contract already initialized");
        initialized = 1;
        minter = _masterChef;
    }
    ...
    /**
     * @dev sets the newMinter address that can be 
     * changed with the update function 14 days after it was set.
     * can only be called by the owner
     */
    function setNewMinter(address _minter) public onlyOwner returns (bool) {
        require(_minter != address(0), "PINE: _minter is the zero address");
        newMinter = _minter;
        updatedAt = block.timestamp;
    }

    /**
     * @dev updates the current minter to the new minter
     * new minter must have been set 14 days before.
     * can only be called by the owner
     */
